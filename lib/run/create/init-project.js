...
export function initProject() {
  ...
  shell.exec(cmd, {async:false}, function(code, stdout, stderr) {
      ...
      view.handleInit = () => {
        ...
        if (mode == "initRepo"){
          if (fs.existsSync(projectPath+ folderSep + name + folderSep + ".git")) {
            command = "cd " + projectPath + more + " mkdir " + name + more + " cd " + name + more + "terosHdlRepo " + "--core " + name
          }
          else{
            command = "cd " + projectPath + more + " mkdir " + name + more + " cd " + name + more + "git init " + more + "terosHdlRepo " + "--core " + name
          }
          atom.notifications.addSuccess("Project initialized")
        }
        else{}

        shell.exec(command, {async:false}, function(code, stdout, stderr) {
          atom.project.addPath(projectPath+folderSep+name)
        });
