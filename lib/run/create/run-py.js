...
class CreateRun {
  ...
  getTb() {
    ...
    dialog.showOpenDialog(
      ...
      function (filenames) {
        ...
        i = 0
        var text = ""
        for (var i=0;i<tb_files_array.length;++i) {
  ...
  createRun() {
    ...
    ////////////////////////////////////////////////////////////////////////////
    var modelRunpy= {
      "src": ["./examples/vhdl/exampleRunpy_1.vhd"],
      "tb": ["./examples/vhdl/tbVhdlVunitRunpy.vhd"],
      "lang": ["vhdl"],
      "config": { "name": "test_1", "outputPath": "./", "codeCovPath": "covHtml", "complex": 1, "poscheck": 1, "precheck": 1 },
      "flags":  { "disableIeeeWarnings": 1, "synopsysLibraries": 1, "pslSupport": 1, "coverageReport": 1 },
      "libraries": { "xilibIseGhdlPath": "xilibIseGhdlPath",
                     "xilibVivadoGhdlPath": "xilibVivadoGhdlPath",
                     "xilibVivadoModelsimPath": "xilibVivadoModelsimPath",
                     "xilibVivadoModelsimPath": "xilibVivadoModelsimPath"}
    }
    modelRunpy['config']['outputPath'] = out_directory[0];
    modelRunpy['config']['name'] = name;
    modelRunpy['src'] = src_files_array;
    modelRunpy['tb'] = tb_files_array;
    // var runpy = new jsteros.Runpy.runpy(modelRunpy);
    var template = new jsteros.Templates.Templates();
    var runpy = template.getTemplate(jsteros.Templates.Codes.TYPES.VUNIT,modelRunpy,"");
    fs.writeFileSync(out_directory[0]+pathLib.sep+name+"_run.py",runpy);
...
export function createRun() {
  ...
  var cmd = "terosHdlRepo --help"
  var shell = require('shelljs');

  shell.exec(cmd, {async:false}, function(code, stdout, stderr) {
    if (code !== 0){
      atom.notifications.addWarning("Please, install correctly TerosHDLbackend.", {detail: "pip install TerosHDL"});
      return;
    }
