...
export function runSimulator(buttonCoverage,consolepanel,gtkwave,params,gtkwaveFile,gtkwaveOutput) {
  ...
  var xsimPath      = exp + "VUNIT_XSIM_PATH=" + atom.config.get('TerosHDL.xsim.xsim-path')
  var vhdl93        = exp + "VUNIT_VHDL_STANDARD=93"
  var python_exec   = " python3 "
  //Exec
  if (gtkwave == false){
    var command = modelsim_path + more + modelsim_ini + more + simulator + more + " cd " + dir + more + python_exec + file + " -x out.xml --exit-0  --no-color --verbose " + params + nameTests
  }
  else if (atom.config.get('TerosHDL.general.simulator')=="modelsim"){
    var command = modelsim_path + more + modelsim_ini + more + simulator + more + ghdlPath + more + " cd " + dir + more + python_exec + file + " -x out.xml --exit-0 --no-color --verbose " + params + nameTests
  }
  else if (atom.config.get('TerosHDL.general.simulator')=="xsim"){
    var command = modelsim_path + more + modelsim_ini + more + simulator + more + ghdlPath + more + xsimPath + more + vhdl93 + more + " cd " + dir + more + python_exec + file + " -x out.xml --exit-0 --no-color --verbose " + params + nameTests
  }
  else {
    var command = modelsim_path + more + modelsim_ini + more + simulator + more + ghdlPath + more + " cd " + dir + more + python_exec + file + " --gtkwave-fmt " + gtkwaveOutput + " -x out.xml --exit-0 --no-color --verbose " + params + nameTests
  }
