...
// Copyright 2019-2020 Carlos Alberto Ruiz Naranjo, Ismael Pérez Rojo
//
...
export function runSimulator(buttonCoverage,consolepanel,gtkwave,params,gtkwaveFile,gtkwaveOutput) {
  ...
  var python = atom.config.get('TerosHDL.general.python')
  ...
  if (gtkwave == false){
    ...
    var command = modelsim_path + more + modelsim_ini + more + simulator + more + ghdlPath + more + xsimPath + more + vhdl93 + more + " cd " + dir + more + ` ${python} ` + file + " -x out.xml --exit-0 --no-color --verbose " + params + nameTests
  }
  else {
