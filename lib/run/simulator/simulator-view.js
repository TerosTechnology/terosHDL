    ...
    for (const i in testsInfo) {
      ...
      if (atom.config.get('TerosHDL.others.sim-display-mode') == "gtkwave"){
        ...
        if (atom.config.get('TerosHDL.general.simulator')=="modelsim"){
          var tWave = doc.createTextNode("Modelsim");
        }
        else{
      ...
      var modelsim_path = "PATH=$PATH:" + atom.config.get('TerosHDL.modelsim.modelsim-path')
      ...
      if (atom.config.get('TerosHDL.others.sim-display-mode') == "gtkwave"){
        btnWAve.onclick = function () {
          ...
          if (atom.config.get('TerosHDL.general.simulator')=="modelsim") {
            // var command = modelsim_ini + ";" + modelsim_path + ";" + "vsim -view " + dir + " -do " + gtkwaveFile
            var command = atom.config.get('TerosHDL.modelsim.modelsim-path') + "/vsim -view " + dir + " -do " + gtkwaveFile
          ...
          // shell.exec('vsim', {async:true})
          // let shell = require('native-exec');
          // let shell = require('native-exec');
          // shell('bash','vsim')
          // shell('/usr/bin/node','vsim')
          // "/usr/bin/node"
        };
      }
...
export function simulatorView(path,pathTests,gtkwaveFile,waveout){
  ...
  xmlhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      var view = new InitSimulatorView(globalInfo,testsInfo,gtkwaveFile);
      const item = {
