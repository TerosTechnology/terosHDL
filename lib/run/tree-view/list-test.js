...
const fs = require('fs')
const ViewConfig = require('./views/ListTests')
...
export default class ListTests {
   constructor(
      ...
      this.getTitle = () => 'TerosHDL tests',
         this.getURI = () => 'atom://terosHDL/tests' + this.idView
   ...
   selectHtml() {
      return {
        table: {
           table: this.element.querySelector('#table-table'),
           load:  this.element.querySelector('#table-load'),
           cancel: this.element.querySelector('#table-cancel'),
           init:  this.element.querySelector('#table-init')
        }
      }
   }
   ...
   // }
   setEvents(){
     this.html.table.cancel.addEventListener('click', () => this.cancel())
