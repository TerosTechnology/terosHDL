...
export default class TreeView {
   ...
   setTop(){
     ...
     if (item_dependencies == activePane.getActiveItem()){
       ...
       for (var i=0;i<childs.length;++i){
         if (childs[i].nodeName == 'title'){
           projectManager.set_top_file(childs[i]['innerHTML']);
           // this.viewDependencies.set_top(childs[i]['innerHTML']);
           return;
         }
       }
       return;
     ...
     // this.viewDependencies.set_top(file_name);
     projectManager.set_top_file(file_name);
     // projectManager.set_top(this.x,this.y);
   }
