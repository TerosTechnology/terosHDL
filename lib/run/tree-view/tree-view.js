...
export default class TreeView {
   ...
   saveProject(){
     ...
     dialog.showSaveDialog(
       ...
       function (path) {
         if(path == undefined)
           return;
         Manager.saveProject(path,project);
       }
     );
   }
   addSource(){
   ...
   refreshTree(items,node){
     for (i=0;i<items.length;++i)
       this.addNodeTree(items[i]['name'],node);
   }
  ...
  play(){
    ...
    //      "simulator": project['config']['tool'],
    //      "language": project['config']['language'],
    //      "module": project['tb'][0],
    //      "files": srcJSON
    //  }
    var edam = Manager.getEdamFormat(project);
    var sim = new jsTeros.Simulators.Manager("localhost","8000");
    this.testView.showLoading();
    var element = this;
    if (project['config']["suite"]=="cocotb"){
      sim.runCocotb(edam).then(function(response) {
      ...
      sim.runEdalize(edam).then(function(response) {
        element.testView.draw(response)
      }, function() {
        // failed
      });
