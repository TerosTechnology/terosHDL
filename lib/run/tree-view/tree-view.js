...
export default class TreeView {
   ...
   configuration(){
     if (this.config == null)
       this.config = new ConfigView(project['config']);
     ...
     this.config.saveConfig = () => {
       this.config.config = {
         ...
         'testName' : this.config.html.text.testName.value,
         'topLevel' : this.config.html.text.topLevel.value
       }
       panel.destroy();
     }
   }
  ...
  play(){
    ...
    for (var i=0;i< project['src'].length;++i) {
     ...
     var manager = new jsTeros.Simulators.Manager("localhost","8000");
