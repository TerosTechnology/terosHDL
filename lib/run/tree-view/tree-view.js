...
export default class TreeView {
   constructor({
      ...
      this.testPanel = atom.workspace.open(this.testView,{split: 'rigth',activatePane:false})
   }
   selectHtml() {
      return {
         btn: {
   ...
   loadProject(){
     ...
     dialog.showOpenDialog(
       ...
       function (path) {
         if(path == undefined)
          return;
         var config = Manager.getConfig(path[0]);
   ...
   saveProject(){
     ...
     dialog.showSaveDialog(
       ...
       function (path) {
         if(path == undefined)
           return;
         Manager.saveProject(path,project['src'],project['tb'],project['config']);
       }
     );
   }
   addSource(){
     ...
     dialog.showOpenDialog(
       ...
       function (src) {
         if(src == undefined)
          return;
         project['src'] = project['src'].concat(Manager.getEdamFileFormat(src));
         element.refreshTree(project['src'],element.html.tree.srcTree);
       }
     );
   }
   addTb(){
     ...
     dialog.showOpenDialog(
       ...
       function (src) {
         if(src == undefined)
           return;
         project['tb'] = project['tb'].concat(Manager.getEdamFileFormat(src));
         element.refreshTree(project['tb'],element.html.tree.tbTree);
       }
     );
   }
   ...
   configuration(){
     var element = this;
     var sim = new jsTeros.Simulators.Manager("localhost","8000");
     sim.getSuites().then(function(resp){
       ...
       element.config.saveConfig = () => {
         element.config.config = {
           ...
           'simulator' : element.config.html.select.simulator.value,
   ...
   refreshTree(items,node){
     for (i=0;i<items.length;++i)
       this.addNodeTree(items[i]['path'],node);
   }
  ...
  play(){
    if(this.config != null)
      project['config'] = this.config.getConfig();
    var srcJSON = project['src'];
    srcJSON = srcJSON.concat(project['tb']);
     var prj = {
         "name": project['config']["name"],
         "working_dir": project['config']["working_dir"],
         "top_level": project['config']["top_level"],
         "simulator": project['config']['simulator'],
    ...
    var element = this;
    if (project['config']["suite"]=="cocotb"){
      sim.runCocotb(prj).then(function(response) {
        element.testView.draw(response)
      }, function() {
        // failed
      });
    }
    else if(project['config']["suite"]=="edalize"){
      sim.runEdalize(prj).then(function(response) {
        element.testView.draw(response)
      }, function() {
        // failed
      });
