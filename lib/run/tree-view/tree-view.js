...
import Output_view from './output-view'
...
export default class TreeView {
   ...
   selectHtml() {
      return {
         btn: {
            ...
            check: this.element.querySelector('#btn-check'),
         },
         tree: {
   ...
   setEvents(){
     ...
     this.html.btn.check.addEventListener('click', () => this.check_project())
   }
   isInDom() {
  ...
  play(){
    ...
    projectManager.simulate(
      ...
      element.testView.showError();
    });
  }
  check_project(){
    let errors_msg = projectManager.check_project()['error_messages'];
    let errors_str = "";
    if (errors_msg.length == 0){
      errors_str = "It's OK :)";
    }
    else{
      for (let i=0; i<errors_msg.length; ++i){
        errors_str += "- " + errors_msg[i] + "\n";
      }
    }
    if (this.check_project_view == null){
      this.check_project_view  = new Output_view(errors_str,"Diagnosis project errors.");
    }
    else{
      this.check_project_view.set_output(errors_str);
    }
    let panel = atom.workspace.open(this.check_project_view,{split: 'rigth',activatePane:true})
  }
