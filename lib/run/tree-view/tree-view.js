...
export default class TreeView {
     ...
     sim.getSuites().then(function(resp){
       ...
       element.config.saveConfig = () => {
         element.config.config = {
           ...
           'testName' : element.config.html.text.testName.value,
           'topLevel' : element.config.html.text.topLevel.value
         }
         panel.destroy();
       }
     });
  ...
  play(){
    if(this.config != null)
      project['config'] = this.config.getConfig();
    var srcJSON = []
    for (var i=0;i< project['src'].length;++i) {
      var element = {
        'path' : project['src'][i]
      };
      srcJSON.push(element);
    }

     var example = {
         "name": project['config']["testName"],
         "top_level": project['config']["topLevel"],
    ...
    var element = this
    sim.runCocotb(example).then(function(response) {
