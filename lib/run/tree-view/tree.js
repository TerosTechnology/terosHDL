...
export default class ProjectManager {
  ...
  load_project(file){
    ...
    this.set_top_file(this.config['top_level_file']);
  ...
  */
  generate_dependency_graph(){
    ...
    var element = this;
    generate_svg(all_sources,this.openFile,this.graph);
    element.set_top_file(this.config['top_level_file']);
  ...
  */
  ...
  set_top(x,y){
    ...
    if (element_mouse.className.includes("Testbenchs")
          || element_mouse.className.includes("Sources")){
      for (var i=0;i<this.element_nodes.length;++i){
        if (this.element_nodes[i]['filepath'] == element_mouse.getAttribute("data-path")){
          this.element_nodes[i]['element_span'].setAttribute("style","font-weight:bold");
          this.set_config_top(element_mouse.getAttribute("data-path"));
        }
        else
          this.element_nodes[i]['element_span'].setAttribute("style","font-weight:normal");
      }
