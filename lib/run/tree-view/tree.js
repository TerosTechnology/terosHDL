...
export default class ProjectManager {
  constructor(source_node,testbench_node,prj_file,graph){
    this.project_manager = new jsTeros.ProjectManager.Manager(graph,null,prj_file);
  ...
  load_project(file){
    ...
    this.update_tree();
    let config = this.project_manager.getConfig();
    this.set_top_from_file(config['top_level_file']);
  ...
  refresh(file){
    if (this.check_if_exist(file) == true)
      this.generate_dependency_graph();
  }
  add_source(source){
    var source_array = [];
  ...
  save_md_doc(output_dir_doc,symbol_vhdl,symbol_verilog,include_dependency_graph){
    this.project_manager.save_md_doc(output_dir_doc,symbol_vhdl,symbol_verilog,include_dependency_graph);
  }
  ...
  /*
    dependency_graph.
  */
  async generate_dependency_graph(){
    let config = this.get_config();
    var all_sources = this.get_all_sources();
    this.project_manager.generate_svg(all_sources,this.openFile,config['top_level_file']);
  }
  ...
  */
  ...
  set_top_default(){
    let config = this.get_config();
    this.set_top_from_file(config['top_level_file']);
    this.project_manager.set_top_dependency_graph(config['top_level_file']);
  }
  ...
  set_top_from_file(file){
    ...
    this.project_manager.set_top_dependency_graph(file);
  }
  ...
  set_config_top(file){
    ...
    try{
      var entity = element.project_manager.get_entity(content,lang);
    }
    catch(error){
      var entity = "";
    }
    let config = element.get_config();
    config['top_level_file'] = file;
    config['top_level'] = entity;
    element.set_config(config);
    element.save_project(element.prj_file);
  }
  check_project(){
    let msg_errors = this.project_manager.check_project();
    return msg_errors;
  }
}
